...
export default async function FoodiesDiscussionThreadPage({
  params,
}: {
  ...
  return (
    <section className="font-app w-full px-4 pb-10 pt-6 sm:px-6 lg:px-8">
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="overflow-hidden rounded-[2rem] border border-white/70 bg-[linear-gradient(135deg,rgba(38,54,26,0.95),rgba(65,84,44,0.88)_56%,rgba(122,152,91,0.78))] px-6 py-8 text-white shadow-[0_28px_80px_-40px_rgba(32,43,21,0.95)] sm:px-8 lg:px-10">
          <p className="text-[0.72rem] font-bold uppercase tracking-[0.34em] text-[#d4e8b9]">
            Foodies Discussion
          </p>
          <Link
            href="/foodies"
            className="mt-5 inline-flex items-center rounded-full border border-white/35 bg-white/15 px-4 py-2 text-xs font-bold uppercase tracking-[0.2em] text-[#e5f4cc] transition hover:bg-white/25 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-white"
          >
          ...
          <p className="mt-2 text-sm text-[#e5f4cc]">
            Started by { thread.postMemberFirstName } on { formatDate(thread.createdAt) }
          </p>
        </div>

        <div className="overflow-hidden rounded-[1.9rem] border border-white/70 bg-white/90 shadow-[0_24px_70px_-40px_rgba(32,43,21,0.75)]">
          ...
          { thread.postsAndReplies.length === 0 ? (
            <div className="px-5 py-5 sm:px-6">
              <InitialPostComposer
                ...
                revalidatePaths={ ["/foodies", `/foodies/discussions/${ thread.id }`] }
              />
            </div>
          ) : (
