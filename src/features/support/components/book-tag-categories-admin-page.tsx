...
import {
  ...
  deleteBookCategoryTagReferenceAction,
  saveBookCategoryAction,
  saveBookCategoryTagReferenceAction,
} from "@/app/(support)/(logged-in)/(books)/add-book-tags/actions";
import {
...
import {
  BookCategory,
  BookCategoryTagReferenceItem,
  BookCategoryWithTags,
...
export function BookTagCategoriesAdminPage({ categories }: { categories: BookCategoryWithTags[] }) {
  ...
  const [editingTag, setEditingTag] = useState<BookCategoryTagReferenceItem | null>(null);
  ...
  function openEditCategoryDialog(category: BookCategory) {
  ...
  function openEditTagDialog(tag: BookCategoryTagReferenceItem) {
    setEditingTag(tag);
    setTagName(tag.tagName);
    setTagCategoryId(tag.bookCategoryId);
    setIsTagDialogOpen(true);
  }

  function handleSaveCategory() {
    startSaveCategory(async () => {
      const result = await saveBookCategoryAction({
  ...
  function handleSaveTag() {
    startSaveTag(async () => {
      ...
      const result = await saveBookCategoryTagReferenceAction({
        id: editingTag?.id,
        bookCategoryId: tagCategoryId,
        tagName,
  ...
  function handleDeleteTag(tag: BookCategoryTagReferenceItem) {
    ...
    startDeleteTag(async () => {
      const result = await deleteBookCategoryTagReferenceAction({ id: tag.id });
  ...
  function handleDeleteCategory(category: BookCategory) {
    ...
    startDeleteCategory(async () => {
      const result = await deleteBookCategoryAction({ id: category.id });
      ...
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="rounded-[1.7rem] border border-[#d8d8dc] bg-[#f8f8f9] px-6 py-6 shadow-[0_24px_60px_-44px_rgba(33,33,38,0.4)] sm:px-8">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div>
              ...
              <h1 className="mt-2 text-3xl font-black tracking-tight text-[#1f1f24]">Book Tag Categories</h1>
              <div className="mt-3">
                <Link
                  href="/"
        ...
        { categories.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-[#c9c9cf] bg-[#f7f7f8] px-6 py-10 text-center text-[#4b4b54]">
            ...
            <p className="mt-2 text-sm">Create your first book tag category to get started.</p>
          </div>
        ) : (
        ...
        <DialogContent className="border-[#d2d2d7] bg-[#f9f9fa] sm:max-w-xl">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#575761]">
              Category name and description are shown as group headings for book tags.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="grid gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input
                  ...
                  placeholder="Classic, Fast-paced, Character-driven..."
                  className="border-[#d0d0d3] bg-white"
                />
              </div>
            </div>
