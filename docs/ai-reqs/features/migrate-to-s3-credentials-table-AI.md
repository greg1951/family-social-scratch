...
- Introduce a server-side credential lookup layer that:
  ...
  - decrypts `encrypted_access_key` and `encrypted_secret_key`,
  - returns `{ accessKeyId, secretAccessKey, bucketName, region }`.
...
- Keep existing seed script for initial population and rotation support.
- Backfill/seed all required family credential rows before enabling strict DB-only reads.
- Rollout in phases:
...
- Credential rotation (`is_active`) switches effective credentials without app redeploy.