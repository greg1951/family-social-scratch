...
Use environment variables so Sentry settings are repeatable across local, staging, and production.

- **SENTRY_DSN** (required): Runtime DSN used by `sentry.server.config.ts` and `sentry.edge.config.ts`.
- **SENTRY_AUTH_TOKEN** (optional, recommended in CI): Needed for source map upload during production builds.
- **SENTRY_ORG** (optional): Sentry organization slug used by the Next.js Sentry build plugin.
- **SENTRY_PROJECT** (optional): Sentry project slug used by the Next.js Sentry build plugin.

Notes:

- Keep these values in environment files or your deployment platform secrets, not hardcoded in source.
- If `SENTRY_ORG` and `SENTRY_PROJECT` are not set as env vars, keep them configured directly in `next.config.ts`.
